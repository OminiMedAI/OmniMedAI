...
class MNASNet(torch.nn.Module):
    ...
    def __init__(self, alpha, in_channels: int = 3, num_classes=1000, dropout=0.2, **kwargs):
        super(MNASNet, self).__init__()
        depths = _scale_depths([24, 40, 80, 96, 192, 320], alpha)
        layers = [
            # First layer: regular conv.
            nn.Conv2d(in_channels, 32, 3, padding=1, stride=2, bias=False),
...
def _load_pretrained(model_name, model, progress):
    if model_name not in _MODEL_URLS or _MODEL_URLS[model_name] is None:
        raise ValueError(
            "No checkpoint is available for core type {}".format(model_name))
